...
По умолчанию заголовки `X-Forwarded-*` не доверяются: все попытки получают общий консервативный лимит. Задайте `VCOBS_TRUST_PROXY_HEADERS=true` только если ваш nginx **перезаписывает**, а не передаёт значения клиента: `proxy_set_header X-Forwarded-For $remote_addr;`, `proxy_set_header X-Forwarded-Host $host;`, `proxy_set_header X-Forwarded-Proto $scheme;`. В этом режиме обязательно укажите `VCOBS_PUBLIC_ORIGIN` как точный публичный origin без пути (например, `https://vcobs.rockxi.ru`). `X-Forwarded-Host` и `X-Forwarded-Proto` должны ему точно соответствовать; иначе запрос отклоняется. Редиректы строятся только по каноническому значению, а клиентский лимит использует валидный IP из `X-Forwarded-For`.
